...
# --- Processamento e Validação de IDs (SINTAXE CORRIGIDA E VERIFICADA) ---
telegram_group_id_int = None
if TELEGRAM_GROUP_ID_STR:
    try: # Bloco if/try/except CORRIGIDO
        telegram_group_id_int = int(TELEGRAM_GROUP_ID_STR)
    except ValueError: # Bloco if/try/except CORRIGIDO
...
async def ajuda_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None: # RENOMEADO
    user = update.effective_user
    ...
    admin_help_text = (
        ...
        "/definir_ciclo <Nome> - (Use no Privado) Define quem é o responsável pelo ciclo ATUAL. Ex: /definir_ciclo Thiago\n\n"
...
async def pago_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if telegram_group_id_int:
        try:
            ...
            await context.bot.send_message(chat_id=telegram_group_id_int, text=group_message, parse_mode=constants.ParseMode.HTML); logger.info(f"Anúncio /pago enviado grupo {telegram_group_id_int}")
        except Exception as group_send_err:
            logger.error(f"Falha ao anunciar /pago grupo {telegram_group_id_int}: {group_send_err}")
...
async def reenviar_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None: # RENOMEADO
    ...
    run_time = datetime.now(tz); month_number = run_time.month; month_name_pt = MESES_PT_BR.get(month_number, f"Mês {month_number}"); year = run_time.year; payer_escaped = escape_html(payer_this_month_name); admin_username_escaped = escape_html("@lucas_gdn"); message = (f"🚨 <b>Lembrete Pagamento Spotify - {escape_html(month_name_pt)}/{year}</b> 🚨\n\n\n👤 Este mês, a vez de pagar é sua: <b>{payer_escaped}</b>\n\n\n<i>(Faça o Pix para o admin ({admin_username_escaped}) e peça para ele confirmar com /pago no privado!)</i>")
    if telegram_group_id_int:
        try:
            await context.bot.send_message(chat_id=telegram_group_id_int, text=message, parse_mode=constants.ParseMode.HTML); logger.info(f"Admin {user.id} reenviou lembrete grupo {telegram_group_id_int}"); await update.message.reply_text(f"✅ Lembrete para <b>{escape_html(payer_this_month_name)}</b> reenviado!", parse_mode=constants.ParseMode.HTML)
        except Exception as send_error:
            logger.error(f"Falha reenviar lembrete grupo {telegram_group_id_int}: {send_error}"); await update.message.reply_text(f"❌ Falha ao reenviar. Erro: {send_error}")
...
async def definir_ciclo_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(f"✅ Ciclo atual definido para: <b>{escape_html(new_current_name)}</b>.\nStatus pgto resetado.\nPróximo job lembrará <b>{escape_html(PAYMENT_ORDER[(new_current_index + 1) % len(PAYMENT_ORDER)])}</b>.", parse_mode=constants.ParseMode.HTML)
...
async def check_spotify_payment(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    month_number = run_time.month; month_name_pt = MESES_PT_BR.get(month_number, f"Mês {month_number}"); year = run_time.year; payer_escaped = escape_html(payer_this_month_name); admin_username_escaped = escape_html("@lucas_gdn"); message = (f"🚨 <b>Lembrete Pagamento Spotify - {escape_html(month_name_pt)}/{year}</b> 🚨\n\n\n👤 Este mês, a vez de pagar é sua: <b>{payer_escaped}</b>\n\n\n<i>(Faça o Pix para o admin ({admin_username_escaped}) e peça para ele confirmar com /pago no privado!)</i>")
    ...
    if telegram_group_id_int:
        try:
            ...
            if admin_user_id_int:
                ...
                except Exception as notify_error: # SINTAXE CORRIGIDA AQUI
                    logger.error(f"Falha ao notificar admin {admin_user_id_int}: {notify_error}")
    else: logger.warning("Job: TELEGRAM_GROUP_ID inválido.")
    bot_state['last_payer_index'] = current_payer_index; save_bot_state(bot_state)

# --- Função Principal ---
def main() -> None:
    """Inicia o bot, handlers e agendador via JobQueue."""
    logger.info(f"Iniciando o bot (vFinal v4 - SINTAXE CORRIGIDA)... Estado inicial: {bot_state}")
    ...
    application.add_handler(CommandHandler("ajuda", ajuda_command)) # RENOMEADO
